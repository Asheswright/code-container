# LLM Coding Container

## Prereqs on Arch

- podman
- podman-compose
- podman-docker

This project is using podman with the podman-docker wrappers.
It should also work with docker, but there might be some slightly different command line arguments.


## Running

Known issue: docker compose seems to cache the container, even when changed or rebuilt with docker build.

```bash
cp sample.env .env
# podman image prune -a --build-cache --external
# docker build --cache-ttl=1h .
docker compose up
ssh -p 2222 arch@localhost
```


